...
As an alternative, we propose that this functional can be adequately represented by a combination of a ConvLSTM with a subsequent FC network. This approach eliminates the need for detailed knowledge of the involved processes and their modeling. Instead, these features can be "learned" by the network in an automated manner. Our proposed network architecture is visualized in @fig-ConvLSTM_FC and described in detail in the following sections. To provide an overview, we will discuss the main components of this architecture one-by-one.
...
Before turning directly to the convolutional LSTM, the simpler architecture of the plain LSTM is examined which serves as a foundation for understanding the more complex ConvLSTM.
...
The choice of atmospheric fields was based on the implemented river runoff calculation in the atmospheric model COSMO-CLM which uses these atmospheric fields to calculate an river runoff estimate.
